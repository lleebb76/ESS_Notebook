...
azure_api_version = "2024-02-01" # Standard stable API version for Azure
...
if not azure_api_key or not azure_endpoint or not azure_deployment:
    st.info("ℹ️ Azure OpenAI credentials not fully set. The ChatGPT option will be disabled.")
        ...
        if st.button("2. Generate Source Overview"):
            with st.spinner(f"Writing Notebook Overview using {ai_choice}..."):
                try:
                    ...
                    elif ai_choice == "ChatGPT (Azure OpenAI)":
...
if user_question := st.chat_input("Ask a question about your sources (or use general knowledge)..."):
    ...
    elif ai_choice == "ChatGPT (Azure OpenAI)" and (not azure_api_key or not azure_endpoint or not azure_deployment):
         st.error("Cannot use ChatGPT. Please add your Azure OpenAI credentials (API Key, Endpoint, and Deployment Name) to the environment variables.")
    else:
            ...
            with st.spinner(f"Analyzing deeply using {ai_choice}..."):
                ...
                try:
                    ...
                    if "limit: 0" in error_msg and "gemini-2.5-pro" in error_msg:
                        ...
                        st.error("🛑 Access Error: You must accept Meta's license agreement on Hugging Face before using Llama 3.")
                    elif "ResourceNotFound" in error_msg and "Azure" in ai_choice:
                        st.error("🛑 Azure Error: The Deployment Name you provided was not found. Please verify it exactly matches the name in Azure OpenAI Studio.")
                    else:
